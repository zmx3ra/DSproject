...
\textbf{Data Quality Notes:} The data had missing values that would cause problems moving into analysis and model construction. In order to address this issue, the missing values were replaced with np.Nan so no additional cleaning was needed. We did not delete missing data so as to not potentially skew end results in later analysis, as well as in model construction. However, later on in the study, some models produced errors after running due to the np.Nan, so we were forced to remove the rows that contained missing data. The number of rows that fell into this category were few, however, and we were still able to use our models on a large portion of our data. The impacts of these deletions should be negligible.

Data cleaning included: coercing columns with periods to numeric as a precaution using pd.to\_numeric(), removing unnecessary wording changing ``30-day Readmission'' to ``Readmission'' using the .str.replace(), splitting the year column to change the format from ``2011-2012'' to ``2011'', and changing the year column to numeric using the pd.to\_numeric(). Changing the year column required a few steps. The year column was split at the ``-'' character using the .st.split() function. Then, as only the start year was needed, not the end year, the first part of the resulting string was selected and kept in a new column, and the old column was deleted. This method was slightly more cumbersome than likely necessary, but it was successful in the end and resulted in a "Year" column that was ready for analysis, visualization, and model construction. Together, all of the changes made data visualization simpler. Additionally, there were no unnecessary dollar signs, commas, or other characters, so no other substantial cleaning was necessary. 
...
To address the second question, a Random Forest model was used in addition to multipl linear regression. The MLR performs poorly as shown by the low r squared value. The random forest, similar to the first question, proved to be the best model to predict the risk adjusted rate. 
