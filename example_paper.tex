...
To address the second question, a Random Forest model was used in addition to multipl linear regression. The MLR performs poorly as shown by the low r squared value. The random forest, similarly to the first question, proved to be the best model for predicting risk adjusted rate. 

\subsection{Defense}

\subsection{Future \& Additional Work}
Future work related to hospital stats and ratings/risk adjusted rate outcomes should expand how many variables the model takes in. With a large dataset that includes multiple data points about a similar subject 
